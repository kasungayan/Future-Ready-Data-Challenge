...
# @output: Energy-forecast-es.csv

# Please uncomment the below commands, in case you haven't installed the following pakcage in your enviornment.
# install.packages("forecast")
# install.packages("smooth")

# Loading the required libraries.
# (https://cran.r-project.org/web/packages/forecast/forecast.pdf)
# https://cran.r-project.org/web/packages/smooth/smooth.pdf
...
for (i in 1:length(cust_id_test)) {
  ...
  # Applying ES function for monthly data (i.e hence the frequency is 12).
  model <- es(ts(df_volume, frequency = 12))
  forecast_model <- forecast(model, h = 8)
  
  # Generating forecasting plots.
  Title <- paste0("ES Forecast Customer ID:", i)
...
write.table(
  forecast,
  file = "Energy-forecast-es123.csv",
  row.names = F,
  col.names = T,
  quote = F
)