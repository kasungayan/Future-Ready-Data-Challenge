...
# @output: Energy-forecast-autoarima.csv
...
for (i in 1:length(cust_id_test)) {
  ...
  # Using the original scale to produce the forecasts.
  model <- auto.arima(ts(df_volume))
  forecast_model <- forecast(model, h = 8)
  
  
  Title <- paste0("ARIMA Forecast Customer ID:", i)
  plot(forecast_model, main =  Title)
  
  # Generating point forecast. (Also, retransforms the original scale after forecasting using exp())
  forecast_values <- as.numeric(exp(forecast_model$mean))
...
write.table(
  forecast,
  file = "Energy-forecast-autoarima.csv",
  row.names = F,
  col.names = T,
  quote = F
)